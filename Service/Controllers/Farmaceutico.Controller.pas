...
function TFarmaceuticoController.ObterFarmaceuticoPorCPF(const ACPF: string): TJSONObject;
var
  lFarmaceutico: TFarmaceutico;
begin
  lFarmaceutico := FFarmaceuticoDAO.BuscarPorCPF(ACPF);
  if Assigned(lFarmaceutico) then
    Result := FMapper.ParaJSON(lFarmaceutico)
  else
    Result := TJSONObject.Create;
...
function TFarmaceuticoController.ObterFarmaceuticoPorCRF(const ACRF: string): TJSONObject;
var
  lFarmaceutico: TFarmaceutico;
begin
  lFarmaceutico := FFarmaceuticoDAO.BuscarPorCRF(ACRF);
  if Assigned(lFarmaceutico) then
    Result := FMapper.ParaJSON(lFarmaceutico)
  else
    Result := TJSONObject.Create;
...
function TFarmaceuticoController.AtualizarFarmaceutico(const AId: Int64; AFarmaceutico: TFarmaceutico): Boolean;
begin
  Result := FFarmaceuticoDAO.Atualizar(AId, AFarmaceutico);
end;

function TFarmaceuticoController.ExcluirFarmaceutico(const AId: Int64): Boolean;
begin
  Result := FFarmaceuticoDAO.Excluir(AId);
end;

function TFarmaceuticoController.CarregarFarmaceutico(const AId: Int64; AFarmaceutico: TFarmaceutico): Boolean;
begin
  Result := FFarmaceuticoDAO.Carregar(AId, AFarmaceutico);
end;
