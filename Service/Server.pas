...
procedure TServer.Iniciar(const Porta: Integer);
begin
...
procedure TServer.OnCommand(AContext: TIdContext; ARequestInfo: TIdHTTPRequestInfo; AResponseInfo: TIdHTTPResponseInfo);
var
  NewPostStream: TMemoryStream;
  OldPosition: Int64;
begin
  try
    if Assigned(ARequestInfo.PostStream) and (ARequestInfo.PostStream.Size > 0) then
    begin
      NewPostStream := TMemoryStream.Create;
      try
        OldPosition := ARequestInfo.PostStream.Position;

        ARequestInfo.PostStream.Position := 0;
        NewPostStream.CopyFrom(ARequestInfo.PostStream, ARequestInfo.PostStream.Size);

        ARequestInfo.PostStream.Free;
        ARequestInfo.PostStream := NewPostStream;

        ARequestInfo.PostStream.Position := OldPosition;
...
procedure TServer.ProcessarRequisicao(ARequestInfo: TIdHTTPRequestInfo; AResponseInfo: TIdHTTPResponseInfo);
begin
  FRouteManager.ProcessarRequisicao(ARequestInfo, AResponseInfo);
end;

procedure TServer.ConfigurarCORS(AResponseInfo: TIdHTTPResponseInfo);
begin
