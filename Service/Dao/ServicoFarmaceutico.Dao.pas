...
type
  ...
  public
    ...
    function ListarTodos: TObjectList<TServicoFarmaceutico>;
...
var
  ...
  lId: Int64;
const
  SQL_UPDATE_CABECALHO = 'UPDATE servicos_farmaceuticos SET ' +
                        'data = :data, ' +
  ...
  SQL_SELECT_IDS_EXISTENTES = 'SELECT id FROM servicos_farmaceuticos_itens ' +
                             'WHERE servico_id = :servico_id ' +
                             'ORDER BY id';

  SQL_UPDATE_ITEM = 'UPDATE servicos_farmaceuticos_itens SET ' +
                   'procedimento_id = :procedimento_id, ' +
                   'valor = :valor ' +
                   'WHERE id = :id';

  SQL_INSERT_ITEM = 'INSERT INTO servicos_farmaceuticos_itens ' +
                   '(servico_id, procedimento_id, valor) ' +
                   'VALUES (:servico_id, :procedimento_id, :valor)';

  SQL_DELETE_ITEM = 'DELETE FROM servicos_farmaceuticos_itens WHERE id = :id';
begin
  Result := False;

  if not Assigned(AServico) or (AServico.Id <= 0) then
    raise EArgumentException.Create('Serviço inválido para atualização');
  ...
  try
    ...
    if not FDB.FDConnection.InTransaction then
      FDB.FDConnection.StartTransaction;

    try
      lQuery.SQL.Text := SQL_UPDATE_CABECALHO;
      lQuery.ParamByName('data').AsDateTime := AServico.Data;
      lQuery.ParamByName('farmaceutico_id').AsLargeInt := AServico.Farmaceutico.Id;
      lQuery.ParamByName('paciente_id').AsLargeInt := AServico.Paciente.Id;
      lQuery.ParamByName('observacoes').AsString := AServico.Observacoes;
      ...
      lQuery.Close;
      lQuery.SQL.Text := SQL_SELECT_IDS_EXISTENTES;
      lQuery.ParamByName('servico_id').AsLargeInt := AServico.Id;
      lQuery.Open;
      ...
      for lItemProcedimento in AServico.ItensProcedimentos do
      begin
        if not Assigned(lItemProcedimento.Procedimento) or
           (lItemProcedimento.Procedimento.Id <= 0) then
          raise EArgumentException.Create('Item com procedimento inválido');

        if lItemProcedimento.Id > 0 then
        begin
          lIdsNovos.Add(lItemProcedimento.Id);

          lQuery.SQL.Text := SQL_UPDATE_ITEM;
          lQuery.ParamByName('id').AsLargeInt := lItemProcedimento.Id;
          lQuery.ParamByName('procedimento_id').AsLargeInt := lItemProcedimento.Procedimento.Id;
          lQuery.ParamByName('valor').AsCurrency := lItemProcedimento.Valor;
          lQuery.ExecSQL;
        end
        else
        begin
          ...
          lItemProcedimento.Id := lQuery.Connection.GetLastAutoGenValue('servicos_farmaceuticos_itens_seq');
          lIdsNovos.Add(lItemProcedimento.Id);
        end;
      end;

      // Remove itens que não estão mais na lista atual
      for lId in lIdsAtuais do
      begin
        if not lIdsNovos.Contains(lId) then
        begin
          lQuery.SQL.Text := SQL_DELETE_ITEM;
      ...
      on E: Exception do
      begin
        if FDB.FDConnection.InTransaction then
          FDB.FDConnection.Rollback;
        raise Exception.CreateFmt('Erro ao atualizar serviço farmacêutico [ID: %d]: %s',
                                [AServico.Id, E.Message]);
      end;
    end;

  finally
    FreeAndNil(lIdsNovos);
    FreeAndNil(lIdsAtuais);
    FreeAndNil(lQuery);
  end;
end;
...
function TServicoFarmaceuticoDAO.BuscarPorId(const AId: Int64): TServicoFarmaceutico;
var
  lQuery: TFDQuery;
  lItemProcedimento: TServicoFarmaceuticoItem;
begin
...
function TServicoFarmaceuticoDAO.ListarTodos: TObjectList<TServicoFarmaceutico>;
var
  lQuery: TFDQuery;
