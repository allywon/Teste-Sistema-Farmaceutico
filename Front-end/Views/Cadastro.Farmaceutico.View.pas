...
begin
  ...
  edtCRF.Text := EmptyStr;
  edtEspecializacao.Text := EmptyStr;
...
begin
  ...
  if Trim(edtCRF.Text) = EmptyStr then
  begin
  ...
  if Trim(edtEspecializacao.Text) = EmptyStr then
  begin
  ...
  if (not FModoEdicao) and (FController.BuscarPorCPF(ptFarmaceutico, Trim(edtCPF.Text)).Id > 0) then
  begin
    ShowMessage('Já existe um farmaceutico cadastrado com esse CPF!');
    edtCPF.SetFocus;
    Exit(False);
  end;

  if (not FModoEdicao) and (FController.BuscarPorCRF(ptFarmaceutico, Trim(edtCRF.Text)).Id > 0) then
  begin
    ShowMessage('Já existe um farmaceutico cadastrado com esse CRF!');
    edtCRF.SetFocus;
    Exit(False);
  end;
