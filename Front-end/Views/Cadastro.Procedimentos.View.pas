...
type
  ...
  private
    ...
    procedure ConfigurarModoEdicao(AEditando: Boolean);
    procedure LimparCampos;
    procedure FiltrarProcedimentos(const ATexto: string);
...
procedure TfCadastroProcedimentos.FiltrarProcedimentos(const ATexto: string);
var
  lTextoPesquisa: string;
begin
  lTextoPesquisa := AnsiLowerCase(ATexto);

  if lTextoPesquisa = EmptyStr then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      ShowMessage('Erro ao salvar procedimento.' + #13 + lProcedimento.Aviso);
  finally
    lProcedimento.Free;
  end;
end;
...
procedure TfCadastroProcedimentos.ConfigurarModoEdicao(AEditando: Boolean);
begin
  FModoEdicao := AEditando;

  btnNovo.Enabled := not AEditando;
  btnEditar.Enabled := not AEditando and not FLstProcedimentos.IsEmpty;
  btnSalvar.Enabled := AEditando;
  btnCancelar.Enabled := AEditando;
  btnExcluir.Enabled := not AEditando and not FLstProcedimentos.IsEmpty;
  edtPesquisa.Enabled := not AEditando;

  dbgProcedimentos.Enabled := not AEditando;
  edtDescricao.Enabled := AEditando;
  edtValor.Enabled := AEditando;
  cmbTipo.Enabled := AEditando;

  edtCodigo.ReadOnly := True;

  if not AEditando then
    LimparCampos;
end;
