...
uses
  ...
  Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.Mask, System.Generics.Collections, System.DateUtils,
  Model.ServicoFarmaceutico, Model.Procedimento, Model.Procedimento.Tipo, Model.Pessoa,
  Controller.ServicoFarmaceutico, Data.DB, Vcl.Grids, Vcl.DBGrids, FireDAC.Comp.Client,
  Vcl.Menus;
...
uses ComboBox.Helper, Controller.Procedimento, Controller.Pessoa;
...
begin
  dtpData.Date := Date;
  TComboBoxHelper.CarregarComboPessoas(cmbFarmaceutico, TPessoaTipo.ptFarmaceutico, 'Selecione um farmaceutico');
  TComboBoxHelper.CarregarComboPessoas(cmbPaciente, TPessoaTipo.ptPaciente, 'Selecione um farmaceutico');
  TComboBoxHelper.CarregarComboProcedimentos(cmbProcedimento, 'Selecione um procedimento');
  PreparaMemtable;
end;
...
begin
  ...
  FreeAndNil(FServicoFarmaceuticoController);
end;
...
var
  ...
  lValor: Currency;
begin
  if (cmbProcedimento.ItemIndex < 1) or (not TryStrToCurr(edtValor.Text, lValor)) then
  begin
...
begin
  if cmbFarmaceutico.ItemIndex < 1 then
  begin
  ...
  if cmbPaciente.ItemIndex < 1 then
  begin
