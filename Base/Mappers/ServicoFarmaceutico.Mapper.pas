...
var
  ...
  lItensProcedimentosJSONArray: TJSONArray;
...
begin
  ...
  try
    ...
    lDataStr := AJSONObject.GetValue<string>('data', EmptyStr);
    if lDataStr <> EmptyStr then
      Result.Data := ISO8601ToDate(lDataStr)
    else
      Result.Data := AJSONObject.GetValue<TDateTime>('data', 0);

    Result.Observacoes := AJSONObject.GetValue<string>('observacoes', EmptyStr);
...
//      Result.Paciente := lPacienteDAO.BuscarPorId(Result.Paciente.Id);
    ...
    if AJSONObject.TryGetValue<TJSONArray>('itens', lItensProcedimentosJSONArray) then
    begin
      for i := 0 to lItensProcedimentosJSONArray.Count - 1 do
      begin
        lItemProcedimentoJSON := lItensProcedimentosJSONArray.Items[i];
...
begin
  ...
  try
    ...
    begin
      Result.AddPair('farmaceutico', TFarmaceuticoMapper.Create.ParaJSON(AServicoFarmaceutico.Farmaceutico));
    end;

    if Assigned(AServicoFarmaceutico.Paciente) then
    begin
      Result.AddPair('paciente', TPacienteMapper.Create.ParaJSON(AServicoFarmaceutico.Paciente));
    end;

    if Assigned(AServicoFarmaceutico.ItensProcedimentos) and (AServicoFarmaceutico.ItensProcedimentos.Count > 0) then
    begin
      lItensProcedimentosJSONArray := TJSONArray.Create;
      ...
      begin
        ...
        lItemProcedimentoJSON.AddPair('procedimento',
          TProcedimentoMapper.Create.ParaJSON(AServicoFarmaceutico.ItensProcedimentos[I].Procedimento)
        );
        lItensProcedimentosJSONArray.AddElement(lItemProcedimentoJSON);
      end;
      Result.AddPair('itens', lItensProcedimentosJSONArray);
    end;
  except
    Result.Free;
    raise;
